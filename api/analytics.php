...
// Optional ?strand=STEM and ?section= filters — narrow every metric below
// to matching students, except strandDistribution/sectionDistribution
// (kept unfiltered so each stays useful as a population-wide breakdown
...
function studentFilterClause(bool $hasStrand, string $strand, bool $hasSection, string $section, string $leadKeyword = 'WHERE'): array
{
    $conds = [];
    $params = [];
    if ($hasStrand) {
...
$totalStudents = (int) (function () use ($pdo, $hasStrand, $strand, $hasSection, $section) {
    [$clause, $params] = studentFilterClause($hasStrand, $strand, $hasSection, $section);
...
$assessedCount = (int) (function () use ($pdo, $hasStrand, $strand, $hasSection, $section) {
    [$clause, $params] = studentFilterClause($hasStrand, $strand, $hasSection, $section, 'AND');
    $stmt = $pdo->prepare(
...
$worksheetCount = (int) (function () use ($pdo, $hasStrand, $strand, $hasSection, $section) {
    [$clause, $params] = studentFilterClause($hasStrand, $strand, $hasSection, $section, 'AND');
    $stmt = $pdo->prepare(
...
$latestRecScores = (function () use ($pdo, $hasStrand, $strand, $hasSection, $section) {
...
$riasecRow = (function () use ($pdo, $hasStrand, $strand, $hasSection, $section) {
    [$clause, $params] = studentFilterClause($hasStrand, $strand, $hasSection, $section, 'AND');
    $stmt = $pdo->prepare(
...
$careerCounts = (function () use ($pdo, $hasStrand, $strand, $hasSection, $section) {
    $needsJoin = $hasStrand || $hasSection;
    [$clause, $params] = studentFilterClause($hasStrand, $strand, $hasSection, $section);
